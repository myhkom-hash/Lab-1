...
if (searchBtn) {
  searchBtn.addEventListener('click', async () => {
    ...
    if (!query) return;
    showMessage('Пошук...', 'info');
    try {
      const data = await apiClient.searchPosts(query);
      state.posts.items = data.items || [];
      renderTable(state.posts.items);
      showMessage(`Знайдено ${state.posts.items.length} результатів`, 'success');
    } catch (err) {
      showMessage('Помилка пошуку: ' + (err.body?.detail || err.message), 'error');
    }
  });
}
...
// --- Сценарій В: IDOR / Broken Access Control + Сценарій Б: XSS ---

const notesUserIdInput = document.getElementById('notesUserIdInput');
...
const notesTableState = document.getElementById('notesTableState');
const notesUserLabel = document.getElementById('notesUserLabel');

let editingNoteId = null;

function showNoteMessage(text, type = 'info') {
  if (!noteStatusMessage) return;
  noteStatusMessage.textContent = text;
  noteStatusMessage.className = `status-message ${type}`;
}


// Дані користувача (note.title, note.content) ніколи не потрапляють в innerHTML
function renderNotesList(notes) {
  if (!notesListContainer) return;
  // Очищуємо контейнер безпечно
  while (notesListContainer.firstChild) {
    notesListContainer.removeChild(notesListContainer.firstChild);
  }

  if (!notes || notes.length === 0) {
    if (notesTableState) notesTableState.textContent = 'Нотаток немає.';
    return;
  }
  if (notesTableState) notesTableState.textContent = '';

  notes.forEach((note) => {
    // Створюємо DOM-елементи, НЕ рядок HTML
    const card = document.createElement('div');
    card.className = 'note-card';
    card.style.cssText = 'border:1px solid #ddd;padding:10px;margin-bottom:8px;border-radius:4px';
    ...
    // textContent: символи <, > відображаються як текст, НЕ як HTML-теги
    ...
    delBtn.addEventListener('click', () => handleDeleteNote(note.id));
    ...
    // textContent: безпечне відображення — навіть <script> буде текстом
    p.textContent = note.content;

    const small = document.createElement('small');
    small.style.color = '#888';
    small.textContent = `ID: ${note.id} · ${formatDateTime(note.createdAt)}`;
...
async function loadNotes() {
  const userId = notesUserIdInput ? notesUserIdInput.value.trim() : '';
  
  // Якщо ID порожній, очищаємо все і виходимо
  if (!userId) {
    if (notesListContainer) notesListContainer.innerHTML = '';
    if (notesTableState) notesTableState.textContent = 'Введіть ваш userId для перегляду нотаток.';
    if (notesUserLabel) notesUserLabel.textContent = '';
    return;
  }

  if (notesUserLabel) notesUserLabel.textContent = `(userId: ${userId})`;
  
  try {
    const data = await apiClient.getNotes(userId);
    ...
    if (notesTableState) {
      notesTableState.textContent = err.body?.detail || err.message || 'Помилка завантаження';
    }
    showNoteMessage(err.body?.detail || err.message || 'Помилка завантаження', 'error');
  }
}
...
function startEditNote(note) {
  ...
  if (noteFormTitle) noteFormTitle.textContent = 'Редагувати нотатку';
  if (noteSubmitBtn) noteSubmitBtn.textContent = 'Оновити';
  if (noteCancelBtn) noteCancelBtn.style.display = 'inline-block';
}
...
function resetNoteForm() {
  ...
  editingNoteId = null;
  const noteFormTitle = document.getElementById('noteFormTitle');
  if (noteFormTitle) noteFormTitle.textContent = 'Моя нотатка';
  if (noteSubmitBtn) noteSubmitBtn.textContent = 'Додати нотатку';
  if (noteCancelBtn) noteCancelBtn.style.display = 'none';
  showNoteMessage('');
}
...
if (noteSubmitBtn) {
  noteSubmitBtn.addEventListener('click', async () => {
    ...
    if (!userId || !title || !content) {
      showNoteMessage('Заповніть userId, заголовок і зміст', 'error');
      return;
    }

    try {
      if (editingNoteId) {
        await apiClient.updateNote(editingNoteId, { title, content }, userId);
        showNoteMessage('Нотатку оновлено', 'success');
      } else {
        await apiClient.createNote({ title, content }, userId);
        showNoteMessage('Нотатку створено', 'success');
      }
      resetNoteForm();
      await loadNotes();
    } catch (err) {
      showNoteMessage(err.body?.detail || err.message, 'error');
    }
  });
}
...
// Завантажуємо нотатки при зміні userId
if (notesUserIdInput) {
  notesUserIdInput.addEventListener('input', loadNotes);
}
...
async function handleDeleteNote(id) {
  ...
  try {
    await apiClient.deleteNote(id, userId);
    await loadNotes();
    showNoteMessage('Нотатку видалено', 'success');
  } catch (err) {
    showNoteMessage(err.body?.detail || err.message, 'error');
  }
}
